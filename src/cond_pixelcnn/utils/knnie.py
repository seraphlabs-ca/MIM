...
def kraskov_mi(x, y, k=5):
    ...
    for i in range(N):
        ...
        ans_x += -digamma(len(tree_x.query_ball_point(x[i], knn_dis[i] -
                                                      1e-15, p=float('inf')))) / N + dx * log(knn_dis[i]) / N
        ans_y += -digamma(len(tree_y.query_ball_point(y[i], knn_dis[i] -
                                                      1e-15, p=float('inf')))) / N + dy * log(knn_dis[i]) / N
...
def kraskov_multi_mi(x, y, z, k=5):
    ...
    # 2*log(N-1) - digamma(N) #+ vd(dx) + vd(dy) - vd(dx+dy)
    ans_xyz = -digamma(k) + digamma(N) + (dx + dy + dz) * log(2)
    ans_x = digamma(N) + dx * log(2)
    ans_y = digamma(N) + dy * log(2)
    ...
    for i in range(N):
        ...
        ans_y += -digamma(len(tree_y.query_ball_point(y[i], knn_dis[i] -
                                                      1e-15, p=float('inf')))) / N + dy * log(knn_dis[i]) / N
        ans_z += -digamma(len(tree_z.query_ball_point(z[i], knn_dis[i] -
                                                      1e-15, p=float('inf')))) / N + dz * log(knn_dis[i]) / N
