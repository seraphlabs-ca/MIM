...
def load_static_mnist(args, **kwargs):
    ...
    with open(os.path.join(datasets_path, 'MNIST_static', 'binarized_mnist_train.amat')) as f:
        lines = f.readlines()
    x_train = lines_to_np_array(lines).astype('float32')
    with open(os.path.join(datasets_path, 'MNIST_static', 'binarized_mnist_valid.amat')) as f:
        lines = f.readlines()
    x_val = lines_to_np_array(lines).astype('float32')
    with open(os.path.join(datasets_path, 'MNIST_static', 'binarized_mnist_test.amat')) as f:
        lines = f.readlines()
...
def load_dynamic_mnist(args, **kwargs):
    ...
    train_loader = torch.utils.data.DataLoader(datasets.MNIST(os.path.join(datasets_path, "mnist"),
    ...
    test_loader = torch.utils.data.DataLoader(datasets.MNIST(os.path.join(datasets_path, "mnist"), train=False,
...
def load_dynamic_fashion_mnist(args, **kwargs):
    ...
    # start processing
    from torchvision import datasets, transforms
    train_loader = torch.utils.data.DataLoader(datasets.FashionMNIST(os.path.join(datasets_path, "fashion-mnist"), train=True, download=True,
                                                                     transform=transforms.Compose([
                                                                         transforms.ToTensor()
                                                                     ])),
                                               batch_size=args.batch_size, shuffle=True)

    test_loader = torch.utils.data.DataLoader(datasets.FashionMNIST(os.path.join(datasets_path, "fashion-mnist"), train=False,
                                                                    transform=transforms.Compose([transforms.ToTensor()
                                                                                                  ])),
                                              batch_size=args.batch_size, shuffle=True)
...
def load_omniglot(args, n_validation=1345, **kwargs):
    ...
    omni_raw = loadmat(os.path.join(datasets_path, 'OMNIGLOT', 'chardata.mat'))
...
def load_caltech101silhouettes(args, **kwargs):
    ...
    def reshape_data(data):
        return data.reshape((-1, 28, 28)).reshape((-1, 28 * 28), order='fortran')
    caltech_raw = loadmat(os.path.join(datasets_path, 'Caltech101Silhouettes', 'caltech101_silhouettes_28_split1.mat'))
...
def load_histopathologyGray(args, **kwargs):
    ...
    # start processing
    with open(os.path.join(datasets_path, 'HistopathologyGray/histopathology.pkl'), 'rb') as f:
...
def load_freyfaces(args, TRAIN=1565, VAL=200, TEST=200, **kwargs):
    ...
    # start processing
    with open(os.path.join(datasets_path, 'Freyfaces/freyfaces.pkl'), 'rb') as f:
        data = pickle.load(f, encoding='latin1')
...
def load_cifar10(args, **kwargs):
    ...
    training_dataset = datasets.CIFAR10(os.path.join(datasets_path, 'cifar10'),
    ...
    # test loader
    test_dataset = datasets.CIFAR10(os.path.join(datasets_path, 'cifar10'), train=False, transform=transform)
