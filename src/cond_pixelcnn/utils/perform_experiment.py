...
def experiment_vae(args, train_loader, val_loader, test_loader, model, optimizer, dir, model_name='vae'):
    from .training import train_vae as train
    from .evaluation import evaluate_vae as evaluate
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            # # EVALUATION
            # if epoch % 10:
            #     test_loss, test_re, test_kl, test_log_likelihood, train_log_likelihood, test_elbo, train_elbo = evaluate(
            #         args, model, train_loader, test_loader, epoch, dir, mode='test')
    except KeyboardInterrupt as e:
        # allow to stop with CTRL-C
        pass

    # FINAL EVALUATION
    best_model = torch.load(dir + args.model_name + '.model')
    test_loss, test_re, test_kl, test_log_likelihood, train_log_likelihood, test_elbo, train_elbo, stats = evaluate(
        args, best_model, train_loader, test_loader, 9999, dir, mode='test')
