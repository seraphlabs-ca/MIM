...
class VAE(Model):
    ...
    def __init__(self, args):
        ...
        self.pixelcnn = nn.Sequential(
            MaskedConv2d('A', self.args.input_size[0] + 2 * self.args.input_size[0],
                         64, 3, 1, 1, bias=False), nn.BatchNorm2d(64), act,
    ...
    def pixelcnn_generate(self, z1, z2):
        ...
        for i in range(self.args.input_size[1]):
            for j in range(self.args.input_size[2]):
                with torch.no_grad():
                    samples_mean, samples_logvar = self.p_x(Variable(x_zeros), z1, z2)
    ...
    def generate_x(self, N=25, return_z=False):
    ...
    # the prior
    def log_p_z2(self, z2, return_a=False):
        if self.args.prior == 'standard':
...
class MIM(VAE):
    ...
    def calculate_loss(self, x, beta=1., average=False):
        ...
        if self.p_samp and (beta >= 1.0):
            ...
            x = self.pixelcnn_generate(z1_q, z2_q)
